...
##============                      首页                        ===================##
def index(request):
    x = ['secondary', 'danger', 'warning','info']
    article = ArticlePost.objects.filter().order_by('-total_views')
    article = Paginator(article, 4)
    article_page = article.get_page(1)
...
def detailedCity(request,id):
    ...
    try:
        ...
        print(get_code.code)
        url = 'http://www.weather.com.cn/data/sk/{0}.html'.format(get_code.code)
...
def post_comment(request,id):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            new_comment.save()
            return redirect('/detailed/{0}'.format(id))
        else:
...
def post_comment_(request,id):
    ...
    if article.id:
        ...
        return redirect('/Article_detail/{0}'.format(id))
...
def User_del(request,id):
    if id == 1:
        if request.method == "GET":
            ...
            a = '/Admin_User/{0}?page={1}'.format(id,page)
...
def Tourism_del(requets,id):
    ...
    print(page)
    return redirect('/Admin_Tourism/{0}'.format(page))
...
def User_car__(request,id):
    ...
    if request.user.id:
        ...
        return redirect('/login/')
    return  redirect('/User_car/?page=1')
...
def AddMony(request,id):
    print(id)
    if request.method == "POST":
        Money = request.POST['money']
        User_edit = User_information.objects.get(user_id=id)
        User_edit.money = Money
        User_edit.save()
        newmoney = User_edit.money
        if User_edit.money == Money:
            return redirect('/search_Scenic/')
...
def Article_Add(request):
    if request.user.id:
        if request.method == "POST":
            ...
            return redirect('/Article_List/{0}'.format(request.user.id))
        else:
